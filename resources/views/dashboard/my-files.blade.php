...
<div class="max-w-7xl mx-auto space-y-6" onclick="closeContextMenu()">
    ...
    <div class="flex items-center justify-between">
        <nav class="flex items-center gap-2 text-xl font-bold text-gray-800">
            <a href="{{ route('my-files') }}" class="hover:underline text-indigo-600">My Files</a>
            @foreach($breadcrumbs as $crumb)
                <span class="text-gray-400 font-normal">/</span>
                <a href="{{ route('my-files.folder', $crumb->id) }}" class="hover:underline text-gray-700">{{ $crumb->folder_name }}</a>
            @endforeach
        </nav>
        
        @if($currentFolder)
            <a href="{{ $currentFolder->parent_id ? route('my-files.folder', $currentFolder->parent_id) : route('my-files') }}" class="px-3.5 py-1.5 text-xs font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center gap-1.5 shadow-sm">
    ...
    <form action="" method="GET" class="flex items-center gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari di berkas saya..." class="w-full pl-11 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
        </div>
    ...
    <div class="flex items-center justify-between pt-4 border-t border-gray-200">
        <div class="flex items-center gap-2">
            <h3 class="text-base font-bold text-gray-800">My Files</h3>
    ...
    <div oncontextmenu="showGlobalContextMenu(event)" class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden min-h-[300px]">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
                ...
                <tr oncontextmenu="showFolderContextMenu(event, {{ $folder->id }}, '{{ $folder->folder_permission }}', true)" class="hover:bg-gray-50/70 transition-colors cursor-pointer select-none">
                    <td class="py-4 px-6 font-medium text-gray-800">
                        <a href="{{ route('my-files.folder', $folder->id) }}" class="flex items-center gap-3 hover:text-indigo-600 group">
                ...
                <tr oncontextmenu="showFileContextMenu(event, {{ $file->id }}, '{{ $file->file_permission }}', true)" class="hover:bg-gray-50/70 transition-colors cursor-pointer select-none">
...
<div id="contextMenu" class="hidden fixed w-52 max-h-[80vh] overflow-y-auto bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 z-50 text-xs">
...
<script>
    ...
    function showFolderContextMenu(e, id, permission, isOwnerOrAdmin) {
        ...
        ctxOpenFolder.href = '/my-files/folder/' + id;
        ctxOpenFile.classList.add('hidden');
        ctxDownloadFile.classList.add('hidden');
        ctxDeleteForm.classList.add('hidden');
